...
Phase 2 tasks into `tasks/manifest.json`, polish the GitHub-facing documents,
...
corroboration)". `NEW_TERMINAL.md` is generalized: two shell variables replace
...
machine-specific paths remaining in `reviews/` and `handoffs/` are evidence
...
**Phase 2 wiring.** All 26 Phase 2 tasks are appended to `tasks/manifest.json`
as waves 6–9 with dependencies as planned, and each has a generated
`tasks/specs/<ID>.md` in the existing spec format. `taskctl.py` already
backfills unseen manifest ids into `.swarm/state.json`, so no state-init change
...
`SWARM_PHASE2_PLAN.md`'s wiring bullet records that this is done.
...
`.swarm/state.json` caveat, and the key-file list from the previous README.
This closes O4 finding N6.

## Files changed

- Loose ends: `release/GATE_EVIDENCE.md`, `NEW_TERMINAL.md`, `CONTRIBUTING.md`,
  `community/ONBOARDING.md`, `community/CAPABILITY_SELF_ASSESSMENT.md`,
  `buzz/canvases/start-here.md`
- Wiring: `tasks/manifest.json`, `tasks/specs/{R1-R4,A1-A5,E1-E5,G1-G4,O1-O5,Q6,Q7,Q-INT}.md` (new),
  `scripts/validate.py`, `scripts/taskctl.py`, `SWARM_PHASE2_PLAN.md`,
  `.swarm/state.json` (ignored local state)
...
- README: `README.md`
- `handoffs/P2-CLEANUP.md` — this file
...
- Phase 2 task metadata was reconstructed from `SWARM_PHASE2_PLAN.md` and the
...
- `.github/ISSUE_TEMPLATE/config.yml` uses `smithdak/practice` absolute URLs
  ...
  default in `OWNER_GATES.md`, not an assertion that gate 3 is cleared; if the
...
- The 26 backfilled `.swarm/state.json` entries record a landing commit but no
