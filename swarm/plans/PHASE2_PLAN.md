...
- Workers never edit `swarm/manifest.json` or `.taskctl/state.json`.
...
- Every task commits and writes `swarm/handoffs/<ID>.md` using `templates/HANDOFF.md`,
...
|---|---|---|---|
| R1 | Refresh the Buzz platform snapshot against live sources | balanced | `buzz/PLATFORM_SNAPSHOT.md` |
| R2 | Currency audit of guide modules and Practices (claims, tool names, as-of dates) | balanced | `reviews/GUIDE_CURRENCY_AUDIT.md` |
| R3 | Adjacent-community landscape scan (what practitioners already have; positioning gaps) | cheap | `docs/founding/COMMUNITY_LANDSCAPE.md` |
| R4 | License, attribution, and claim re-audit of all publishable artifacts | balanced | `reviews/CLAIM_AUDIT.md` |
...
| O4 | Re-run the onboarding simulation with Phase 2 artifacts; record results | balanced | `reviews/ONBOARDING_DRY_RUN_PHASE2.md` |
...
| Q7 | Fact and currency re-audit of research outputs | balanced | `reviews/FACT_AUDIT_PHASE2.md` |
| Q-INT | Integrate accepted findings, update `OWNER_REVIEW.md` evidence rows, run release validation | strong | `release/OWNER_REVIEW.md`, `swarm/reports/PHASE2_REPORT.md` |
...
- Wiring: **done.** All 26 tasks are in `swarm/manifest.json` as waves 6–9 with
  a spec under `swarm/specs/`, so `taskctl.py` and
  `scripts/validate.py --task <ID>` recognize them. `taskctl.py` backfills any
  manifest id it has not seen into `.taskctl/state.json`, so no state-init change
