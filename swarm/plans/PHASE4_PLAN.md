...
[non-goals](../../docs/NON_GOALS.md), not a tooling change, and this phase does not make
it.
...
| ID | Title | Owned outputs |
|---|---|---|
| W2 | Wire the substrate, extend the ladder and CI, record the phase | `ops/OPERATING_LOOP.md`, `docs/framework/AUTONOMY_LADDER.md`, `tests/test_contract_integration.py`, `swarm/reports/PHASE4_REPORT.md` |
...
- Workers never edit `swarm/manifest.json` or `.taskctl/state.json`, and never run
...
- Every task writes `swarm/handoffs/<ID>.md` using [templates/HANDOFF.md](../../templates/HANDOFF.md).
